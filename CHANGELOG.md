...
- **Control panel: language and temperature-unit rows overlapped** (2.6.2): the 2.5.0 unit picker was placed at the language row's exact frame coordinates; the language row now sits on its own line and everything below shifted down accordingly.
- **Charging power excluded from the thermal reading** (2.6.1): `SystemPowerIn` is adapter input and includes battery charging — 30 W+ during fast charge, none of which reaches the heatsink. Plugging in after a drained meeting pushed the reading to 62 W and the feedforward toward max RPM on a near-idle machine (live capture: 54 W input, 50.4 W charging, true thermal ≈ 8.7 W). The daemon now parses charge current × voltage from the same ioreg call and subtracts 90 % of it (the remaining ~10 % is real conversion heat); the UI shows charging separately as "⚡ N W".
...
- **控制面板语言行与温度单位行重叠**（2.6.2）：2.5.0 加温度单位选择器时直接复用了语言行的坐标，两行完全叠在一起；现语言行独立成行，其下元素相应下移。
- **充电功率不再计入温控口径**（2.6.1）：`SystemPowerIn` 是适配器输入、含电池充电功率——快充时 30W+，一瓦都不经过散热片。开会耗完电插回电源后，读数被顶到 62W、前馈在近乎空闲的机器上把风扇往满转拉（实测：输入 54W，其中充电 50.4W，真实发热口径仅 8.7W）。daemon 现从同一次 ioreg 调用解析充电电流×电压并减去其 90%（余下 ~10% 是真实的转换发热）；界面上充电功率以"⚡ N W"单独标注。
